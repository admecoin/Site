...
const NftProvider: React.FC<NftProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const fetchContractData = async () => {
      try {
        const polyearnRabbitsContract = getPolyearnRabbitContract()
        ...
        const balanceOf = await polyearnRabbitsContract.methods.balanceOf(account).call()
        ...
        if (balanceOf > 0) {
          const getTokenIdAndBunnyId = async (index: number) => {
            try {
              const tokenId = await polyearnRabbitsContract.methods.tokenOfOwnerByIndex(account, index).call()
              const bunnyId = await polyearnRabbitsContract.methods.getBunnyId(tokenId).call()
